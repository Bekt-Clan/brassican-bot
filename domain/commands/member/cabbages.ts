...
import { CabbageCommandParameters } from '../../../commands/types';
...
const getUserName = (member: User) => {
    ...
    console.log('Global name:', member.globalName);
    console.log(
        'Nickname:',
        member.client.guilds.cache
            .get(Environment.GUILD_ID)
            ?.members.cache.get(member.id)?.nickname
    );
    return member.username || member.globalName;
    // return member.username || member.globalName || member.nickname!;
};

const mobileBreakdown = (member: User, memberData: IMember) => {
    ...
    const nickname = getUserName(member);
...
const cabbageEmbed = (member: User, memberData: IMember) => {
    const client = getDiscordClient();
    const { accountProgression: account } = memberData;
...
export const cabbages = async (
    interaction: ChatInputCommandInteraction,
    parsedOptions: CabbageCommandParameters
) => {
    await interaction.deferReply({ flags: 'Ephemeral' });

    const member =
        parsedOptions.member || (interaction.member as GuildMember | null);
